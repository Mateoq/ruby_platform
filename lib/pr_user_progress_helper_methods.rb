class PrUserProgressHelperMethods
  ...
  def get_total_grade(metadata, with_type = false)
    num_items = 0
    total_grade = 0
    metadata.each do |_index, item|
      next unless item[:type] == CourseData.lesson_types[:activity] if with_type
  ...
  def update_general_progress(course_progress, options = {})
    ...
    course_progress_metadata[:progress][options[:pr_guide]][options[:pr_lesson].to_sym].merge!(grade: total_grade,
                                                                                               done: true)
    ...
    update_course_progress(course_progress, cache_name)
  end
end
