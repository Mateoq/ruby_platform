class PrHelperMethods
  ...
  def init_course(course_class, course_grade, parent_id, is_lesson = false, options = {})
    ...
    unless query_course = Rails.cache.fetch("#{@session_data[:user_token]}_course_#{cache_name}") || course
      query_course = progress_model.init_data(base_user_info.merge(name: cache_name,
                                                                   current_grade: course_num,
                                                                   pr_type: UserProgress.progress_types[:course],
                                                                   metadata: user_course_progress.to_json), lesson: is_lesson)
    ...
    (0..3).each do |i|
      unless query = Rails.cache.fetch("#{@session_data[:user_token]}_guide_#{cache_name}_#{'%02d' % i}")
        query = progress_model.init_data(base_user_info.merge(name: "#{cache_name}_#{'%02d' % i}",
                                                              pr_type: UserProgress.progress_types[:guide],
                                                              parent_id: query_course.id), lesson: is_lesson)
    ...
    lessons.each_with_index do |lesson, _i|
      ...
      unless query = Rails.cache.fetch("#{@session_data[:user_token]}_lesson_#{cache_name}_#{'%02d' % lesson_metadata[:guide]}_#{'%02d' % lesson_metadata[:lesson_num]}")
        ...
        query = progress_model.init_data(base_user_info.merge(name: "#{cache_name}_#{'%02d' % lesson_metadata[:guide]}_#{'%02d' % lesson_metadata[:lesson_num]}",
                                                              pr_type: UserProgress.progress_types[:lesson],
                                                              parent_id: query_keys[lesson_metadata[:guide]],
                                                              enabled: enabled,
                                                              current: false), lesson: is_lesson)
  ...
  def init_lesson_progress(options = {})
    ...
    lesson_items = Rails.cache.fetch(cache_name, expires_in: 24.hours) do
      CourseData.where(course_id: options[:data][:lesson_id]).order(:order)
    end
    ...
    unless lesson_progress.key?(options[:data][:lesson_app].to_sym)
    ...
    lesson_items.each_with_index do |i, key|
      ...
      unless lesson_progress[options[:data][:lesson_app].to_sym].key?(i[:url_name].to_sym)
        save = true
  ...
  def init_lesson(course_structure, _course_lesson)
    lesson_data = Rails.cache.fetch(course_structure[:lesson_app], expires_in: 24.hours) do
      lesson_structure = CourseData.where(course_id: course_structure[:lesson_id]).order(:pr_type, :order)

      data = []
      date_strings = %w(created_at updated_at)
  ...
  def restore_course(course_class, course_grade, options = {})
    ...
    user_progress_cache_name = "#{@session_data[:user_token]}_course_#{course_class}_#{'%02d' % course_grade_num}"

    user_progress = Rails.cache.fetch(user_progress_cache_name)
    ...
    Rails.cache.write(user_progress_cache_name, user_progress, expires_in: 24.hours)
  ...
  def get_js_lesson_data(user_progress, options = {})
    ...
    if options[:lesson]
      ...
      user_progress[:lesson_progress][options[:app].to_sym].each do |k, p|
        ...
        display_name = (CourseData.lesson_types[:concepts] == p[:type]) ? k.to_s.sub(/\-/, ' ').capitalize : p[:name]
  ...
  def format_slider_items(lesson_progress)
    ...
    lesson_progress.each do |_key, value|
      slider_carousel[a_num] ||= []
